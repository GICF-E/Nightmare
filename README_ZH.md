...
### 血量的回复
在FPS-Demo中，目前回复血量的唯一方法是食用食物。如果您想要通过食物来回复血量，您需要拾取食物（方法详见`/操作模式 - 物体的丢弃和拾取/`），角色会在您拾取后自动食用食物。在食用食物的过程中，您将听到声音提示，血量会在食用过程中不断增加。目前FPS-Demo已支持多种食物，每种食物可以增加的血量和咀嚼的时间各不相同。
